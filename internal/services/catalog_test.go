...
var _ = Describe("Services", func() {
	Describe("Plan Metadata Retrieval", func() {
		Context("with a valid plan.yml file", func() {
			It("can read the plan metadata", func() {
				p, err := services.ReadPlan("../../test/ok/redis/small")
		...
		Context("without a plan.yml file", func() {
			It("throws an error", func() {
				_, err := services.ReadPlan("../../test/bad/missing/small")
		...
		Context("without a plan directory", func() {
			It("throws an error", func() {
				_, err := services.ReadPlan("../../test/bad/missing/enoent")
				Ω(err).Should(HaveOccurred())
			})
		})
	})
	...
	Describe("Service Metadata Retrieval", func() {
		Context("with a valid service.yml file", func() {
			It("can read the service metadata", func() {
				service, err := services.ReadService("../../test/ok/redis")
				Ω(err).ShouldNot(HaveOccurred())
				Ω(service).ShouldNot(BeNil())
				Ω(service.ID).Should(Equal("redis-1"))
				Ω(service.Name).Should(Equal("redis"))
				Ω(service.Description).Should(Equal("something awesome"))
				Ω(service.Bindable).Should(BeTrue())

				Ω(service.Tags).Should(HaveLen(2))
				Ω(service.Tags[0]).Should(Equal("foo"))
				Ω(service.Tags[1]).Should(Equal("bar"))

				Ω(service.Plans).Should(HaveLen(3))
		...
		Context("without a service.yml file", func() {
			It("throws an error", func() {
				_, err := services.ReadPlan("../../test/bad/empty")
				Ω(err).Should(HaveOccurred())
			})
		})
		...
		Context("without a service directory", func() {
			It("throws an error", func() {
				_, err := services.ReadPlan("../../test/bad/absent")
				Ω(err).Should(HaveOccurred())
			})
		})
		...
		Context("with a malformed child plan", func() {
			It("throws an error", func() {
				_, err := services.ReadPlan("../../test/bad/missing")
				Ω(err).Should(HaveOccurred())
			})
		})
	})
	...
	Describe("Catalog Metadata Retrieval", func() {
		...
		Context("with invalid search paths", func() {
			It("throws an error", func() {
				_, err := services.ReadServices("../../test/ok", "../../test/enoent")
				Ω(err).Should(HaveOccurred())
			})
		})
	...
	Describe("Service Limits", func() {
		GiveMeAPlan := func(serviceLimit, planLimit int) services.Plan {
		...
		GiveMeAnIndex := func(inPlan, outOfPlan, outOfService int) *vaultPkg.Index {
			database := &vaultPkg.Index{
				Data: make(map[string]interface{}),
			}

			for n := range inPlan {
				name := fmt.Sprintf("in-plan-%d", n)
				database.Data[name] = map[string]interface{}{
		...
		Context("with no limits", func() {
			It("can handle any number of existing instances without going over-limit", func() {
				...
				for i := 0; i < 100; i = 100 {
					for j := 0; j < 100; j = 100 {
						for k := 0; k < 100; k = 100 {
							database := GiveMeAnIndex(i, j, k)
							Ω(plan.OverLimit(database)).Should(BeFalse())
						}
					}
				}
			})
		...
		Context("with only service limits", func() {
			...
			It("is not over-limit when the combined number of service-instances is less than the limit", func() {
				...
				database = GiveMeAnIndex(0, 2, 0)
				Ω(plan.OverLimit(database)).Should(BeFalse())

				database = GiveMeAnIndex(2, 2, 0)
				Ω(plan.OverLimit(database)).Should(BeFalse())
			})
			...
			It("is over-limit when the combined number of service-instances equals the limit", func() {
				...
				database = GiveMeAnIndex(0, 5, 0)
				Ω(plan.OverLimit(database)).Should(BeTrue())

				database = GiveMeAnIndex(3, 2, 0)
				Ω(plan.OverLimit(database)).Should(BeTrue())
			})
			...
			It("is over-limit when the combined number of service-instances exceeds the limit", func() {
				...
				database = GiveMeAnIndex(0, 6, 0)
				Ω(plan.OverLimit(database)).Should(BeTrue())

				database = GiveMeAnIndex(3, 3, 0)
				Ω(plan.OverLimit(database)).Should(BeTrue())
			})
		...
		Context("with service and plan limits", func() {
			...
			It("is over-limit when the number of service-instances exceeds the service-limit", func() {
				database := GiveMeAnIndex(4, 4, 0)
				Ω(plan.OverLimit(database)).Should(BeTrue())
			})

			It("is not over-limit when another service exceeds the limit", func() {
				database := GiveMeAnIndex(2, 2, 200)
				Ω(plan.OverLimit(database)).Should(BeFalse())
			})
		})
	})
