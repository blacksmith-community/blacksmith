...
func (e *PluginsExecutorService) ExecuteCommand(ctx context.Context, execCtx PluginsExecutionContext, deployment, instance string, index int, category, command string, arguments []string) (*PluginsStreamingExecutionResult, error) {
	...
	// Validate arguments
	err = e.metadataService.ValidateCommand(command, arguments)
	if err != nil {
	...
	fullCommand := e.buildRabbitMQPluginsCommand(command, arguments)

	// Execute command asynchronously
	go e.executeCommandAsync(ctx, execCtx, result, deployment, instance, index, fullCommand, cmd.Timeout)
...
func (e *PluginsExecutorService) ExecuteCommandSync(ctx context.Context, execCtx PluginsExecutionContext, deployment, instance string, index int, category, command string, arguments []string) (string, int, error) {
	...
	rabbitCmd := RabbitMQCommand{
		Name:        fullCommand,
		Args:        arguments,
		Description: "rabbitmq-plugins " + command,
	...
	if err != nil {
		e.logger.Errorf("Failed to execute rabbitmq-plugins command: %v", err)
	...
	e.logger.Infof("Rabbitmq-plugins command completed with exit code: %d", sshResult.ExitCode)
...
func (e *PluginsExecutorService) GetPluginStatus(listOutput string) map[string]string {
	...
	for _, line := range lines {
		...
		if len(matches) >= PluginMatchParts {
...
func (e *PluginsExecutorService) buildRabbitMQPluginsCommand(command string, arguments []string) string {
...
func (e *PluginsExecutorService) sendCommandInfo(result *PluginsStreamingExecutionResult, instance string) {
...
func (e *PluginsExecutorService) sendProcessedOutput(result *PluginsStreamingExecutionResult, processedOutput string) {
...
func (e *PluginsExecutorService) handleCommandResult(result *PluginsStreamingExecutionResult, err error, sshResult *RabbitMQCommandResult, timeout int, commandCtx context.Context) {
	endTime := time.Now()
...
func (e *PluginsExecutorService) executeCommandAsync(ctx context.Context, _ PluginsExecutionContext, result *PluginsStreamingExecutionResult, deployment, instance string, index int, fullCommand string, timeout int) {
	defer close(result.Output)

	result.Status = StatusRunning
	e.sendCommandInfo(result, instance)

	// Create context with timeout
	timeoutDuration := time.Duration(timeout) * time.Second
	commandCtx, cancel := context.WithTimeout(ctx, timeoutDuration)
	...
	rabbitCmd := RabbitMQCommand{
		...
		Description: "rabbitmq-plugins " + result.Command,
		Timeout:     timeout,
	}
	...
	e.sendProcessedOutput(result, processedOutput)

	// Handle the result
	e.handleCommandResult(result, err, sshResult, timeout, commandCtx)
	...
	e.logger.Infof("Rabbitmq-plugins command execution completed: %s (exit code: %d, duration: %v)",
