...
var (
	ErrInvalidFilePath = errors.New("invalid file path: contains directory traversal")
)

// Note: The planstore package itself is tested through integration tests
// These are helper functions for test scenarios

// Helper function for calculating SHA256 in tests.
...
func TestStoreVaultPaths(t *testing.T) {
	t.Parallel()
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			...
			vaultPath := fmt.Sprintf("plans/%s/%s/%s/%s", testCase.service, testCase.plan, testCase.fileType, testCase.sha256)
			if vaultPath != testCase.expected {
				t.Errorf("Expected vault path %s, got %s", testCase.expected, vaultPath)
			}
		})
	}
}
...
func TestInstancePlanReferencePaths(t *testing.T) {
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			...
			vaultPath := fmt.Sprintf("%s/%s", instanceID, testCase.refType)
			if vaultPath != testCase.expected {
				t.Errorf("Expected vault path %s, got %s", testCase.expected, vaultPath)
			}
		})
