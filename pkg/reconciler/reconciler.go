...
// buildInstanceFromCF creates instance data primarily from CF service instance information
func (r *reconcilerManager) buildInstanceFromCF(cfInstance CFServiceInstanceDetails) *InstanceData {
	// Find service and plan information from broker catalog
	var serviceName, planName, serviceType string
	...
	// Build comprehensive metadata starting with CF data
	metadata := map[string]interface{}{
		// CF-specific data
		"service_name":      cfInstance.ServiceName,
		"service_type":      serviceType,
		...
		"reconciliation_source": "cf_primary",
		"reconciled_at":         time.Now().Format(time.RFC3339),
		"cf_discovery_time":     time.Now().Format(time.RFC3339),
	...
	// Use CF instance GUID as the instance ID (this is the authoritative ID)
	instanceID := cfInstance.GUID

	// If we couldn't find service/plan in broker catalog, infer from CF data
	if serviceID == "" {
	...
	if serviceName == "" {
		serviceName = cfInstance.ServiceName
	}
	if planName == "" {
		planName = cfInstance.PlanName
	}
	...
	return &InstanceData{
		...
		ServiceID:      serviceID,
		PlanID:         planID,
		DeploymentName: r.inferDeploymentName(cfInstance), // Will be updated if BOSH deployment found
		Metadata:       metadata,
...
// enrichInstanceWithBOSH adds BOSH deployment information to CF-based instance data
func (r *reconcilerManager) enrichInstanceWithBOSH(instance *InstanceData, boshDeployment DeploymentInfo) {
	// Get detailed BOSH information if available
	...
	// Update deployment name and manifest
	instance.DeploymentName = boshDeployment.Name
	instance.Manifest = detail.Manifest

	// Enrich metadata with BOSH data
	if instance.Metadata == nil {
		instance.Metadata = make(map[string]interface{})
	}

	// Add BOSH-specific metadata
	instance.Metadata["bosh_deployment_name"] = boshDeployment.Name
	instance.Metadata["bosh_releases"] = detail.Releases
	instance.Metadata["bosh_stemcells"] = detail.Stemcells
	instance.Metadata["bosh_vms"] = detail.VMs
	instance.Metadata["bosh_teams"] = detail.Teams
	instance.Metadata["bosh_variables"] = detail.Variables
	instance.Metadata["bosh_properties"] = detail.Properties
	instance.Metadata["bosh_created_at"] = boshDeployment.CreatedAt.Format(time.RFC3339)
	instance.Metadata["bosh_updated_at"] = boshDeployment.UpdatedAt.Format(time.RFC3339)
	instance.Metadata["reconciliation_source"] = "cf_and_bosh"
	...
	if detail.Properties != nil {
		if taskID, ok := detail.Properties["latest_task_id"].(string); ok && taskID != "" {
			instance.Metadata["latest_task_id"] = taskID
		}
	}

	r.logDebug("Enriched CF service instance %s with BOSH deployment data from %s", instance.ID, boshDeployment.Name)
}

// inferDeploymentName tries to infer the BOSH deployment name from CF service instance data
func (r *reconcilerManager) inferDeploymentName(cfInstance CFServiceInstanceDetails) string {
	// Service deployments follow the pattern: {service-name}-{plan-name}-{instance-guid}
	...
	serviceName := cfInstance.ServiceName
	planName := cfInstance.PlanName
	instanceGUID := cfInstance.GUID

	// The most likely pattern is: {service-name}-{plan-name}-{instance-guid}
	// This matches the examples you provided
	if serviceName != "" && planName != "" && instanceGUID != "" {
		deploymentName := fmt.Sprintf("%s-%s-%s", serviceName, planName, instanceGUID)
		r.logDebug("Inferred deployment name: %s for CF instance %s", deploymentName, instanceGUID)
		return deploymentName
	}

	// Fallback: if we don't have complete CF data, try other patterns
	if planName != "" && instanceGUID != "" {
		deploymentName := fmt.Sprintf("%s-%s", planName, instanceGUID)
		r.logDebug("Inferred deployment name (fallback): %s for CF instance %s", deploymentName, instanceGUID)
		return deploymentName
	}

	// Last resort: just use the instance GUID
	r.logDebug("Using instance GUID as deployment name: %s", instanceGUID)
	return instanceGUID
}
