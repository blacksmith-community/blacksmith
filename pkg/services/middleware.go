...
func NewSecurityMiddleware(logger func(string, ...interface{})) *SecurityMiddleware {
	return &SecurityMiddleware{
		rateLimiter: common.NewRateLimiter(common.DefaultRateLimit, time.Minute), // requests per minute
...
func (sm *SecurityMiddleware) ValidateRequest(instanceID, operation string, params map[string]interface{}) error {
	...
	// Input validation
	err := common.ValidateInputs(params)
	if err != nil {
...
func (sm *SecurityMiddleware) HandleSecurityError(writer http.ResponseWriter, err error) bool {
	var serviceErr *common.ServiceError
	if errors.As(err, &serviceErr) && serviceErr.ServiceType == "security" {
...
func (sm *SecurityMiddleware) GetRateLimitHeaders(instanceID, operation string) map[string]string {
	...
	return map[string]string{
		"X-RateLimit-Limit":     strconv.Itoa(sm.rateLimiter.Limit),
